...
def normal_extrude(excount, o):
    ...
    while count < excount:
        ...
        bpy.ops.transform.translate(
            value=(2 * random.random(), 2 * random.random(), o),
            orient_type="GLOBAL",
...
def lightning_mesh(location=(0, 0, 0)):
    ...
    bpy.ops.mesh.primitive_cube_add(
        size=5, enter_editmode=True, align="WORLD", location=(0, 0, 0)
    )
    count = 0
    ...
    bpy.context.object.modifiers["Displace"].keyframe_insert(
        data_path="strength", frame=1
    )
...
def add_object_button(self, context):
    self.layout.operator(
        AddLightningObject.bl_idname, text="Add Lightning Object", icon="MOD_NOISE"
    )
